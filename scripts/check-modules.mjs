...
for (const match of serviceWorker.matchAll(/'(\.\/[^']+)'/g)) {
  ...
  // Anything under data/ is generated by the build rather than committed, so it
  // is absent from a clean checkout by design. Only one such file used to exist,
  // and this check named it explicitly, which meant adding a second one failed
  // the build for no good reason.
