...
/** SHA-256 over bytes, returning lowercase hex. */
export function sha256Bytes(bytes) {
  ...
  view.setUint32(padded.length - 4, bitLen % 4294967296);
 ...
 *
 * Two builds of the same cards must produce identical bytes, so that a digest
 * can be compared by a human or a machine years apart. Anything that would
 * make the output vary between machines is rejected rather than serialised.
 */
export function canonical(value) {
  ...
  if (type === 'number') {
    if (!Number.isFinite(value)) throw new TypeError('canonical: non-finite number');
  ...
  throw new TypeError(`canonical: unsupported value of type ${type}`);
}

/** The digest of a single term card, which is what a citation should name. */
export function cardDigest(card) {
  return sha256Hex(canonical(card));
}

/** The digest of a whole ordered set of cards: the term base's revision. */
export function baseDigest(cards) {
  const ordered = [...cards].sort((a, b) => a.id.localeCompare(b.id));
  return sha256Hex(ordered.map((card) => `${card.id}:${cardDigest(card)}`).join('\n'));
}

export const short = (hex, length = 12) => hex.slice(0, length);
